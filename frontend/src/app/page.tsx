...
export default function Home() {
    ...
    useEffect(() => {
        ...
        api.get("/doctors")
            ...
            .catch(() => {
                if (!cancelled) setFeaturedDoctors([]);
            })
            .finally(() => {
    ...
    return (
        <div className="min-h-screen bg-[#fcfcfb] text-[#171717]">
            <Navbar />
            <main className="landing-page w-full overflow-x-clip pb-8">
                <section className="relative grid overflow-hidden border-x border-b border-[#dedede] bg-[#f8fbff] px-6 pb-14 pt-12 sm:px-10 lg:grid-cols-[1.05fr_.95fr] lg:px-14 lg:pb-20 lg:pt-16">
                    ...
                    <div className="relative z-10 max-w-155 min-w-0">
                    ...
                    <div className="relative mt-12 flex min-h-70 items-end justify-center lg:mt-0 lg:min-h-97.5"><div className="absolute bottom-0 h-64 w-64 rounded-full bg-[#dbeafe] sm:h-80 sm:w-80" /><div className="relative z-10 flex h-64 w-52 items-end justify-center rounded-t-[7rem] bg-[#b8d7ff] sm:h-80 sm:w-64"><div className="absolute bottom-0 h-40 w-44 rounded-t-[6rem] bg-[#2563eb] sm:h-48 sm:w-52" /><div className="absolute bottom-36 size-28 rounded-full bg-[#f1b28f] sm:bottom-44 sm:size-32" /><div className="absolute bottom-[13.8rem] h-12 w-36 rounded-t-full bg-[#253b67] sm:bottom-68" /><div className="absolute bottom-[13.2rem] left-16 h-1.5 w-1.5 rounded-full bg-[#1f2937] sm:bottom-[16.4rem] sm:left-20" /><div className="absolute bottom-[13.2rem] right-16 h-1.5 w-1.5 rounded-full bg-[#1f2937] sm:bottom-[16.4rem] sm:right-20" /><div className="absolute bottom-[11.7rem] h-5 w-10 rounded-b-full border-b-2 border-[#c46f65] sm:bottom-[14.8rem]" /></div><div className="absolute bottom-4 right-0 z-20 rounded-lg border border-[#dbe5f2] bg-white p-3 text-xs shadow-lg sm:right-4"><div className="flex items-center gap-2 font-semibold text-white"><HeartPulse className="size-4 text-[#10b981]" /> </div><p className="mt-1 text-[#89919b]">Talk to our AI assistant</p></div></div>
                </section>
                ...
                <section id="doctors" className="border-x border-b border-[#cfe0f5] bg-[#f4f9ff] px-6 py-10 sm:px-10 sm:py-12">
                    <SectionHeading number="5" color="blue" title="Featured Doctors - Preview" subtitle="Meet trusted doctors 
                        ...
                        {featuredLoading ? (
                            <div className="mt-8 grid gap-4 md:grid-cols-3">
                                {[1, 2, 3].map((i) => (
                                    <div key={i} className="rounded-2xl border border-[#cfe0f5] bg-white p-5 shadow-sm animate-pulse">
                                        <div className="flex items-start justify-between">
                                            <div className="h-16 w-16 rounded-2xl bg-[#e8f1ff]" />
                                            <div className="h-6 w-20 rounded-full bg-[#e8f1ff]" />
                                        </div>
                                        <div className="mt-5 h-5 w-2/3 rounded bg-[#e8f1ff]" />
                                        <div className="mt-2 h-4 w-1/3 rounded bg-[#e8f1ff]" />
                                        <div className="mt-3 h-4 w-full rounded bg-[#e8f1ff]" />
                                    </div>
                                ))}
                            </div>
                        ) : featuredDoctors.length === 0 ? (
                            <p className="mt-8 rounded-2xl border border-dashed border-[#cfe0f5] bg-white p-8 text-center text-sm text-[#60758b]">
                                Featured doctors will appear here once doctor profiles are available.</p>
                        ) : (
                            <div className="mt-8 grid gap-4 md:grid-cols-3 cursor-pointer">{featuredDoctors.map((doctor) => <div key={doctor.id} className="group rounded-2xl border border-[#cfe0f5] bg-white p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-[#8db8ed] hover:shadow-lg hover:shadow-[#2563eb]/10"><div className="flex items-start justify-between"><div className="flex size-16 items-center justify-center overflow-hidden rounded-2xl bg-[#e8f1ff] text-base font-bold text-[#2563eb] shadow-inner">{doctor.image ? <img src={doctor.image} alt={doctor.name} className="size-full object-cover" /> : <span>Dr</span>}</div><span className="inline-flex items-center gap-1 rounded-full bg-[#e8f1ff] px-2 py-1 text-[10px] font-semibold text-[#2563eb]"><CheckCircle2 className="size-3" /> Verified</span></div><h3 className="mt-5 font-bold text-[#173b68]">{doctor.name}</h3><p className="mt-1 text-sm text-[#5f7489]">{doctor.specialty}</p><div className="mt-3 flex items-center justify-between"><p className="flex items-center gap-1 text-xs font-semibold text-[#e59b21]"><Star className="size-3.5 fill-current" /> {doctor.rating ?? "—"} <span className="font-normal text-[#8da0b6]">patient rating</span></p><span className="text-xs text-[#6c91ba]">{doctor.experience ? `${doctor.experience}+ years` : "Available now"}</span></div><Link href={`/doctors/${doctor.id}`} className="mt-5 block rounded-lg border border-[#2563eb] py-2.5 text-center text-sm font-semibold text-[#2563eb] hover:text-[#5f8ae9] transition-colors">View profile</Link></div>)}</div>
                        )}
                </section>

                <section className="border-x border-b border-[#dedede] bg-white px-6 py-10 sm:px-10 sm:py-12"><div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-[#1d4ed8] via-[#2563eb] to-[#4f46e5] px-6 py-10 text-center text-white shadow-xl shadow-[#2563eb]/20 sm:px-10"><div className="absolute -right-16 -top-20 size-52 rounded-full border border-white/20" /><div className="absolute -bottom-24 -left-10 size-56 rounded-full border border-white/15" /><div className="relative"><span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold text-blue-100"><Sparkles className="size-3.5" /> Your care journey starts here</span><h2 className="mx-auto mt-4 max-w-2xl text-2xl font-bold tracking-[-0.02em] sm:text-3xl">Ready to get started? Join thousands of patients today.</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-blue-100">Take the first step toward clearer health decisions. Chat with MedNoviAI, explore trusted doctors, and book your appointment when you are ready.</p><div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row"><Link href={user ? (user.role === "doctor" ? "/doctor/dashboard" : "/patient/dashboard") : "/signup"} className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-white px-5 text-sm font-semibold text-[#2563eb] shadow-sm transition-colors hover:bg-blue-50">{user ? `Go to Dashboard (${user.name})` : "Register for Free"} <ArrowRight className="size-4" /></Link>{!user && <Link href="/login" className="inline-flex h-11 items-center justify-center rounded-lg border border-white/60 px-5 text-sm font-semibold text-white transition-colors hover:bg-white/10">Login</Link>}</div></div></div></section>
                <FAQSection />
            </main>
            <Footer />
        </div>
